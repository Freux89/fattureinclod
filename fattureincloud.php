...
class fattureincloud extends Module
{
    ...
    */
    public function __construct()
    {
        ...
        $this->version = '2.1.2';
        $this->author = 'fatureincloud mod Cristian';
    ...
    */
    public function install()
    {
        ...
        // $this->registerHook('additionalCustomerFormFields'); // manca
        $this->registerHook('actionCustomerAccountAdd');
        // $this->registerHook('actionCustomerAccountUpdate'); // manca
        
        // Hooks for additional customer fields on back-end
        // $this->registerHook('actionCustomerFormBuilderModifier'); // manca
        // $this->registerHook('actionAfterCreateCustomerFormHandler'); // manca
        // $this->registerHook('actionAfterUpdateCustomerFormHandler'); // manca
            
        // // Hooks for additional tax field on back-end
        // $this->registerHook('actionTaxFormBuilderModifier'); // manca
        // $this->registerHook('actionAfterCreateTaxFormHandler'); // manca
        // $this->registerHook('actionAfterUpdateTaxFormHandler'); // manca
    ...
    */
    public function installTab()
    {
        ...
        $tab->name[$this->context->language->id] = 'FattureInCloud';
    ...
    */
    public function getContent()
    {
       ...
       // Tools::addJsDef(array('admin_controller_link' => $admin_controller_link));
       $this->context->smarty->assign('admin_controller_link', $admin_controller_link);
    ...
    {
        $query = 'SELECT a.`fic_client_id` , a.pec, a.cod_interscambio'
    ...
    public function hookActionOrderStatusPostUpdate($order)
    {
        ...
        if (Configuration::get('FATTUREINCLOUD_INVOICES_CREATE')
            ...
            && ($order_complete->current_state == Configuration::get('PS_OS_PAYMENT')
                || $order_complete->current_state == Configuration::get('PS_OS_WS_PAYMENT') || $order_complete->current_state == 15)
            ) {
    ...
    public function composeInvoice($order_id)
    {
        ...
        // Cristian recupero l'indirizzo per ottenere il tipo di documento richiesto
        $document = 0;
        ...
        if ($order->id_address_invoice) {
            ...
            $document = $address->documento;
        }
        
        // Cristian : 1 = fattura; 0 = ricevuta
        if($document == '1'){ 
        ...
        if (Configuration::get('FATTUREINCLOUD_INVOICES_SEND_TO_SDI')) {
            $invoice_to_create['data']['e_invoice'] = true;
            $invoice_to_create['data']['ei_data'] = $this->getEIPaymentCodeByModule($order->module);
        }
        }
        else{
           $invoice_to_create['data']['type'] = "receipt";
        
        if (Configuration::get('FATTUREINCLOUD_RECEIPTS_SUFFIX')) {
            $invoice_to_create['data']['numeration'] = Configuration::get('FATTUREINCLOUD_RECEIPTS_SUFFIX');
        }
     ...
     public function getEIPaymentCodeByModule($payment_module)
     {
         ...
         if ($payment_module == 'ps_cashondelivery') { 
             ...
             if (!empty($payment_module_details->details)) {
                 $ei_payment['bank_iban'] = str_replace(' ', '', $payment_module_details->details);
             }
         } 
    ...
    public function mapPrestashopToFicDocument($order_id)
    {
        ...
        $billing_address_state = new State($billing_address->id_state);
        ...
        if (isset($fic_address_fields['pec'])) {
            $entity['certified_email'] = $fic_address_fields['pec'];
        }
        
        if (isset($fic_address_fields['cod_interscambio'])) {
            $entity['ei_code'] = $fic_address_fields['cod_interscambio'];
        }
        ...
        foreach ($products as $product) {
            ...
            $product_orig = new Product($product['id_product']);
            $item['name'] = $product_orig->name[6];
            $item['qty'] = $product['product_quantity'];
            if (!empty($product['reduction_percent']) && floatval($product['reduction_percent']) > 0) {
                $item['gross_price'] = $product['unit_price_tax_excl'] + ($product['unit_price_tax_excl'] * 22 / 100);
        ...
        if ($order->total_shipping > 0) {
            ...
            //     $item = array();
            //     $item['name'] = "Spedizione: " . $carrier->name;
            //     $item['gross_price'] = $real_shipping_cost;
            //     $item['vat'] = array();
            //     $item['vat']['id'] = $fic_carrier_vat_id;
            //     $items[] = $item;
            ...
            //     $item['gross_price'] = $other_costs_net;
            //     $item['vat'] = array();
            //     $item['vat']['id'] = $fic_carrier_vat_id;
            //     $items[] = $item;
            // }
        ...
        if ($order->total_wrapping > 0) {
            ...
            if ($calculated_wrapping_cost == $order->total_wrapping_tax_incl) {
                $item = array();
                $item['name'] = "Imballo";
                $item['gross_price'] = $order->total_wrapping_tax_excl;
                $item['vat'] = array();
                $item['vat']['id'] = $fic_carrier_vat_id;
                $items[] = $item;
            } else {
                ...
                $item = array();
                $item['name'] = "Imballo";
                $item['gross_price'] = $real_wrapping_cost;
                $item['vat'] = array();
                $item['vat']['id'] = $fic_carrier_vat_id;
                $items[] = $item;
        ...
        // MP05: Bonifico ; // MP01 Contanti; // MP08 Carta di pagamento
        if($order->payment == 'Bank wire'){
            $cod_payment_method = 'MP05';
        }
        else if($order->payment == 'Pagamento alla consegna (COD)'){
            $cod_payment_method = 'MP01';
        }
        else{
            $cod_payment_method = 'MP08';
        }
        $document = array(
            "data" => array(
                "currency" => $currency,
