...
class FattureInCloudClient
{
    ...
    private $scope = "entity.clients:a issued_documents.orders:a issued_documents.invoices:a issued_documents.receipts:a settings:a products:a";
    ...
    private function makeCompanyRequest($endpoint, $body = null, $method = "GET")
    {
        ...
        while ($makeRequest && $retry <= $this->maxRetry) {
            ...
            $response = curl_exec($ch);
            // verifico gli endpoint
            $urll =  $this->apiBaseUrl . '/c/' . $this->companyId . '/'. $endpoint;
            
            $sql = "INSERT INTO `ps_log_fattureincloud` (`endpoint`,`response`,`ch`) VALUES ('$urll','$response','$ch')";
